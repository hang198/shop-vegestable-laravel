<?php

namespace App;

class Cart
{
    public $items = null;
    public $totalQty = 0;
    public $totalPrice = 0;

    public function __construct($oldCart)
    {
        if ($oldCart) {
            $this->items = $oldCart->items;
            $this->totalQty = $oldCart->totalQty;
            $this->totalPrice = $oldCart->totalPrice;
        }
    }

    public function add($item, $id, $qtyProduct = 1)
    {

        if ($item->promotion_price) {
            $storedItem = ['qty' => 0, 'price' => $item->promotion_price, 'item' => $item];
        } else {
            $storedItem = ['qty' => 0, 'price' => $item->unit_price, 'item' => $item];

        }

        if ($this->items) {
            if (array_key_exists($id, $this->items)) {
                $storedItem = $this->items[$id];
            }
        }

        if ($qtyProduct) {
            $storedItem['qty'] += $qtyProduct;
        } else {
            $storedItem['qty']++;
        }
        $storedItem['price'] = $item->unit_price * $storedItem['qty'];
        $this->items[$id] = $storedItem;
        $this->totalQty++;


        if ($qtyProduct) {
            if ($item->promotion_price) {
                $this->totalPrice += $item->promotion_price * $qtyProduct;
            } else {
                $this->totalPrice += $item->unit_price * $qtyProduct;
            }

        } else {
            if ($item->promotion_price) {
                $this->totalPrice += $item->promotion_price;
            } else {

                $this->totalPrice += $item->unit_price;
            }
        }




    }

    //xóa 1
    public function reduceByOne($id)
    {
        $this->items[$id]['qty']--;
        $this->items[$id]['price'] -= $this->items[$id]['item']['price'];
        $this->totalQty--;
        $this->totalPrice -= $this->items[$id]['item']['price'];
        if ($this->items[$id]['qty'] <= 0) {
            unset($this->items[$id]);
        }
    }

    //xóa nhiều
    public function removeItem($id)
    {
        $this->totalQty -= $this->items[$id]['qty'];
        $this->totalPrice -= $this->items[$id]['price'];
        unset($this->items[$id]);
    }

}
